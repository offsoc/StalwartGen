use std::{
    fmt::{Display, Formatter}, // 引入Display和Formatter用于格式化输出
    time::Duration, // 引入Duration用于时间处理
};

use hyper::{header::AUTHORIZATION, HeaderMap}; // 引入hyper库中的HeaderMap和AUTHORIZATION
use ring::signature::{UnparsedPublicKey, ED25519}; // 引入ring库中的签名模块

use base64::{engine::general_purpose::STANDARD, Engine}; // 引入base64库用于编码和解码
use store::write::now; // 引入store库中的now函数
use trc::ServerEvent; // 引入trc库中的ServerEvent

use crate::manager::fetch_resource; // 引入当前crate中的fetch_resource函数

const LICENSING_API: &str = "https://license.stalw.art/api/license/"; // 许可证API URL
const RENEW_THRESHOLD: u64 = 60 * 60 * 24 * 4; // 4 days // 续订阈值

// 定义LicenseValidator结构体，用于验证许可证
pub struct LicenseValidator {
    public_key: UnparsedPublicKey<Vec<u8>>, // 公钥
}

// 定义LicenseKey结构体，用于表示许可证密钥
#[derive(Debug, Clone)]
pub struct LicenseKey {
    pub valid_to: u64, // 有效期至
    pub valid_from: u64, // 有效期从
    pub domain: String, // 域名
    pub accounts: u32, // 账户数量
}

// 定义LicenseError枚举，用于表示许可证错误
#[derive(Debug)]
pub enum LicenseError {
    Expired, // 过期
    InvalidDomain { domain: String }, // 无效域名
    DomainMismatch { issued_to: String, current: String }, // 域名不匹配
    Parse, // 解析错误
    Validation, // 验证错误
    Decode, // 解码错误
    InvalidParameters, // 无效参数
    RenewalFailed { reason: String }, // 续订失败
}

// 定义RenewedLicense结构体，用于表示续订的许可证
pub struct RenewedLicense {
    pub key: LicenseKey, // 许可证密钥
    pub encoded_key: String, // 编码后的密钥
}

const U64_LEN: usize = std::mem::size_of::<u64>(); // u64的长度
const U32_LEN: usize = std::mem::size_of::<u32>(); // u32的长度

// 为LicenseValidator结构体实现方法
...
// 为LicenseError枚举实现Display trait
