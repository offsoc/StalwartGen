    ...
    pub fn log_license_details(&self) {
        if let Some(enterprise) = &self.core.enterprise {
            trc::event!(
                ...
                ValidFrom = DateTime::<Utc>::from_timestamp(enterprise.license.valid_from as i64, 0).to_rfc3339(),
                ValidTo = DateTime::<Utc>::from_timestamp(enterprise.license.valid_to as i64, 0).to_rfc3339(),
            );
        }
    }
